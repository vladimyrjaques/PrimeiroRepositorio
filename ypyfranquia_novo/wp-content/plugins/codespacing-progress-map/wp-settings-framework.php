    ...
    class CsPm_WordPressSettingsFramework {
    	...
    	function cspm_admin_enqueue_scripts()
    	{
			...
			if (is_admin()) {
				...
				$plugin_settings = cspm_wpsf_get_settings( $this->plugin_path .'settings/codespacing-progress-map.php' );		
				
				$api_key = isset($plugin_settings['codespacingprogressmap_mapsettings_api_key']) ? $plugin_settings['codespacingprogressmap_mapsettings_api_key'] : '';		
		...
		function cspm_get_registred_cpt($secondary = false){
			...
			if(!$secondary){
				...
				$post_type_field = array(
					...
					'desc' => 'Select in the post types list the post type for which Progress Map should be available during post creation/edition. (Default: '.__('Posts').').<br />
							   <span style="color:red"><strong style="font-size:14px;"><u>How to display the map of the Main content type in both the content of the page/post or by putting it directly in your PHP page?</u></strong><br /><br />
							   
							   <strong>1)</strong> To display the map of the main post type in your page/post content, you simply need to put the shortcode <strong>[codespacing_progress_map]</strong> in the content. It is preferable to use the Text Editor 
							   of the content instead of the Visual Editor to avoid any problem of adding a strange character to the shortcode.<br /><br />
							   
							   <strong>2)</strong> To display the map of the main post type directly in your PHP page, make use of the following code: <br /><pre class="cspm_pre">&lt;?php echo do_shortcode("[codespacing_progress_map]"); ?&gt;</pre></span>',
					'type' => 'select',
				...
				$post_type_field = array(
					...
					'title' => 'Secondary content types',
					'desc' => 'Select the other content types for which Progress Map should be available during post creation/edition. 
					  		   The selected CPTs wont be used in the main query, you can call them later in the other instances of the map.<br />
							   <span style="color:red"><strong style="font-size:14px;"><u>How to display the map of a Secondary content type in both the content of the page/post or by putting it directly in your PHP page?</u></strong><br /><br />
							   
							   <strong>1)</strong> To display the map of a secondary post type, you need to use the shortcode attribute <strong>post_type</strong>.
							   The value of this attribute must be the Post Type Name (lowercase name with no spaces) that you can find between brackets in the above list.<br />
							   <strong>Example: [codespacing_progress_map post_type="page"]</strong><br />
							   It is preferable to use the Text Editor of the content instead of the Visual Editor to avoid any problem of adding a strange character to the shortcode.<br /><br />
							   
							   <strong>2)</strong> To display the map of a secondary post type directly in your PHP page, make use of the following code: <br /><pre class="cspm_pre">&lt;?php echo do_shortcode("[codespacing_progress_map post_type="page"]"); ?&gt;</pre></span>',
					'type' => 'multi_select',
					'choices' => $post_types_array,
				);
		...
		function cspm_get_post_taxonomies(){
			...
			$this_settings = cspm_wpsf_get_settings( $this->plugin_path .'settings/codespacing-progress-map.php' );		
			
			$post_type = isset($this_settings['codespacingprogressmap_generalsettings_post_type']) ? $this_settings['codespacingprogressmap_generalsettings_post_type'] : 'post';		
		...
		function cspm_get_markers_taxonomy_terms($post_taxonomies, $faceted_search = false){
			...
			if(isset($this_settings['codespacingprogressmap_markercategoriessettings_marker_taxonomies'])){
				
				$marker_taxonomy = $this_settings['codespacingprogressmap_markercategoriessettings_marker_taxonomies'];		
